...
- `raw_text` or `rawText` (string): Additional freeform description
 - `reporting_id`, `reportingId`, or `trackingId` (string): Optional reporting identifier generated by the device. If provided the server will attempt to use this as the report's tracking ID.
...
The API now returns a simple acknowledgement after persisting the report. Devices should not expect internal IDs in the response.
